...
usage: plot_cadence_power.py [-h] [--title TITLE] [-b] [--csv CSV] [-g GLOBAL_OFFSET] [--start START] [--stop STOP] [-c CUSTOM [CUSTOM ...]] [-a A_TRAINING_TRACKER [A_TRAINING_TRACKER ...]] [-f FIT_FILES [FIT_FILES ...]]
                             [--scale-a SCALE_A]
...
options:
  ...
  --csv CSV             If provided, processes and writes the data to CSV files instead . The provided string is used as a prefix. (default: None)
...
Importers:
  ...
  -c CUSTOM [CUSTOM ...], --custom CUSTOM [CUSTOM ...]
                        Folders containing data generated by log_power_meter.py. Data is formatted as a series of tuples of FOLDERNAME[,DISPLAY_NAME,OFFSET]]. If the display name is not provided, the folder name will be
                        used and the offset set to 0. The offset (in seconds) defaults to 0 if not specified. (default: None)
  -a A_TRAINING_TRACKER [A_TRAINING_TRACKER ...], --a-training-tracker A_TRAINING_TRACKER [A_TRAINING_TRACKER ...]
                        CSV file generated by the A Training Tracker App (or similar). Data is formatted as a series of sets of FOLDERNAME[,DISPLAY_NAME,[OFFSET]]. If the display name is not provided, the file name will be
                        used and the offset set to 0. The offset (in seconds) defaults to 0 if not specified. (default: None)
  -f FIT_FILES [FIT_FILES ...], --fit-files FIT_FILES [FIT_FILES ...]
                        FIT file to import. Data is formatted as a series of sets of FOLDERNAME[,DISPLAY_NAME,[OFFSET]]. If the display name is not provided, the folder name will be used and the offset set to 0. The offset
                        (in seconds) defaults to 0 if not specified. (default: None)
  --scale-a SCALE_A     Scalar value to scale the data from a training tracker. This is useful if the power meter logged through this is set to the incorrect crank length. (default: 1)
...
def save_csvs(importers: List[Importer], prefix:str) -> None:
    """Saves all data to CSV files, adjusted for the time offsets.

    Args:
        importers (List[Importer]): The importers to save.
        prefix (str): The prefix to add to the exported filename.
    """
    # Create a folder if needed
    if "/" in prefix or "\\" in prefix:
        # We have a folder path in the name.
        folder = os.path.dirname(prefix)
        if not os.path.exists(folder):
            print(f"Folder '{folder}' does not exist. Will create it now.")
            os.makedirs(folder)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--csv",
        help="If provided, processes and writes the data to CSV files instead . The provided string is used as a prefix.",
        type=str,
        default=None
    )
    ...
    if not len(importers):
        ...
        if args.csv is not None:
            # Save to a CSV file instead of plotting
            print("Saving the results to csv files instead of plotting")
            save_csvs(importers, args.csv)
        else:
